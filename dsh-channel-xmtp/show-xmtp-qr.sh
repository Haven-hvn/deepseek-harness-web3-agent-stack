...
try_npx() {
  if has_cmd npx && has_cmd node; then
    # qrcode-terminal prints a block QR to stdout
    echo "  QR (npx qrcode-terminal):"
    echo ""
    # --yes avoids interactive prompt on first fetch
    npx --yes qrcode-terminal@0.12.0 "${DM_URL}" 2>/dev/null && { echo ""; return 0; }
    # fallback: inline node using qrcode lib
    node --input-type=module <<'NODE' 2>/dev/null && { echo ""; return 0; }
import qrcode from 'qrcode-terminal';
qrcode.generate(process.env.DM_URL, { small: true });
NODE
  fi
  return 1
}
